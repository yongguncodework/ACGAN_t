...
def build_generator(latent_size):
    ...
    image_class = Input(shape=(2,), dtype='int32')

    # 10 classes in CIFAR-10
    cls = Flatten()(Embedding(10, 55,
...
def build_discriminator():
    ...
    fake = Dense(2, activation='sigmoid', name='generation',
                 kernel_initializer='glorot_normal', bias_initializer='Zeros')(features)
...
if __name__ == '__main__':
    ...
    for epoch in range(nb_epochs):
        ...
        for index in range(nb_batches):
            ...
            sampled_labels1 = np.random.randint(0, class_num, size= (batch_size, 1))
            sampled_labels2 = (sampled_labels1 - 1) * (-1)
            sampled_labels = np.reshape([sampled_labels1, sampled_labels2], (16, 2))
            ...
            # disc_real_weight = [np.ones((batch_size, 2)), 2 * np.ones((batch_size, 2))]
            # disc_fake_weight = [np.ones((batch_size, 2)), np.zeros((batch_size, 2))]
            ...
            for train_ix in range(3):
                if index % 30 != 0:
                    ...
                    y_real = np.random.uniform(0.0, 0.3, size=(batch_size, 2))
                    # aux_y1 = label_batch.reshape(-1, )
                    aux_y1 = label_batch

                    chec1 = [y_real, aux_y1]
                    chec2 = np.reshape([y_real, aux_y1], (16, 4))
                    # epoch_disc_loss.append(discriminator.train_on_batch(X_real, [y_real, aux_y1]))
                    epoch_disc_loss.append(discriminator.train_on_batch(X_real, np.reshape([y_real, aux_y1], (16, 4))))
                    # Label Soomthing
                    X_fake = generated_images
            ...
            sampled_labels1 = np.random.randint(0, class_num, 2 * batch_size)
            sampled_labels2 = (sampled_labels1 - 1) * (-1)
            sampled_labels = np.reshape([sampled_labels1, sampled_labels2], (16, 2))
        ...
        sampled_labels = y_test[0:16, 0:1]
