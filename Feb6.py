...
def build_generator(latent_size):
    ...
    cls = Flatten()(Embedding(label_dim, 25,
...
def build_discriminator():
    ...
    # cnn.add(GaussianNoise(0.05, input_shape=12288))  # Add this layer to prevent D from overfitting!
    ...
    # cls = Flatten()(Embedding(label_dim, 3*32*32,
    #                           embeddings_initializer='glorot_normal')(image_class))

    cls = Flatten()(Embedding(2, np.prod([3, 32, 32]))(image_class))
    # cls_res = np.reshape(cls, [3, 64, 64])
    image_flat = Flatten()(image)

    h = layers.multiply([image_flat, cls])
    ...
    aux = Dense(4, activation='sigmoid', name='auxiliary',
                kernel_initializer='glorot_normal', bias_initializer='Zeros')(features)

    return Model([image, image_class], [fake, aux])
...
if __name__ == '__main__':
    ...
    zeros_for_label = np.zeros((880, 2))
    labels = np.concatenate([labels, zeros_for_label], axis=1)
    ...
    fake, aux = discriminator([fake, image_class])
    ...
    y_train_one = labels[:, 0:label_dim]

    X_test = X_train[0:100, :, :, :]
    y_test = y_train[0:100, 0:label_dim]
    y_test_one = y_train[0:100, 0:label_dim]
    nb_train, nb_test = X_train.shape[0], X_test.shape[0]
    ...
    for epoch in range(nb_epochs):
        ...
        for index in range(nb_batches):
            ...
            label_batch_one = y_train_one[index * batch_size:(index + 1) * batch_size]
            # YG change
            # label_batch = y_train[index * batch_size:(index + 1) * batch_size, :]

            # sample some labels from p_c
            sampled_labels_disc = np.random.randint(0, 2, size=batch_size)
            sampled_labels_disc = to_categorical(sampled_labels_disc, label_dim)

            sampled_labels = np.random.normal(0, 0.3, size=(batch_size, 4))
            ...
            for train_ix in range(3):
                if index % 30 != 0:
                    X_real = image_batch
                    # Label Soomthing
                    y_real = np.random.uniform(0.7, 1.2, size=(batch_size, 1))
                    # aux_y1 = label_batch.reshape(-1, )
                    aux_y1 = label_batch_one
                    epoch_disc_loss.append(discriminator.train_on_batch([X_real, aux_y1], [y_real, aux_y1]))
                    # Label Soomthing
                    X_fake = generated_images
                    y_fake = np.random.uniform(0.0, 0.3, size=(batch_size, 1))
                    aux_y2 = sampled_labels_disc

                    # see if the discriminator can figure itself out...
                    epoch_disc_loss.append(discriminator.train_on_batch([X_fake, aux_y2], [y_fake, aux_y2]))
                else:
                    # make the labels the noisy for the discriminator: occasionally flip the labels
                    # when training the discriminator
                    X_real = image_batch
                    y_real = np.random.uniform(0.0, 0.3, size=(batch_size, 1))
                    # aux_y1 = label_batch.reshape(-1, )
                    aux_y1 = label_batch_one
                    ...
                    # Label Soomthing
                    X_fake = generated_images
                    y_fake = np.random.uniform(0.7, 1.2, size=(batch_size, 1))
                    # aux_y2 = sampled_labels_disc
                    aux_y2 = sampled_labels_disc
            ...
            sampled_labels = np.random.normal(0, 0.3, size=(2 * batch_size, 4))

            # sampled_labels_disc = np.random.randint(0, 2, size=batch_size)
            # sampled_labels_disc = to_categorical(sampled_labels_disc, label_dim)

            sampled_labels_disc1 = np.random.randint(0, class_num, batch_size)
            sampled_labels_disc1 = to_categorical(sampled_labels_disc1, label_dim)

            sampled_labels_disc2 = np.random.randint(0, class_num, batch_size)
            sampled_labels_disc2 = to_categorical(sampled_labels_disc2, label_dim)

            sampled_labels_disc = np.concatenate([sampled_labels_disc1, sampled_labels_disc2], axis=0)

            # sampled_labels_disc = np.reshape(sampled_labels_disc, [64, 4])
            ...
            epoch_gen_loss.append(combined.train_on_batch(
                [noise, sampled_labels], [trick, sampled_labels_disc]))
        ...
        # sampled_labels_disc = np.random.randint(0, class_num, nb_test)
        # sampled_labels_disc = to_categorical(sampled_labels_disc, label_dim)
        sampled_labels = np.random.normal(0, 0.3, size=(nb_test, 4))
        # sampled_labels = y_test[0:nb_test, 0:label_dim]
        sampled_labels_disc = y_train_one[0:nb_test, 0:label_dim]
        generated_images = generator.predict(
        ...
        aux_y = np.concatenate((y_test_one, sampled_labels_disc), axis=0)

        # see if the discriminator can figure itself out...
        discriminator_test_loss = discriminator.evaluate(
            [X, aux_y], [y, aux_y], verbose=False)
        ...
        # sampled_labels = to_categorical(sampled_labels, label_dim)
        sampled_labels = np.random.normal(0, 0.3, size=(2 * nb_test, 4))
        trick = np.ones([2 * nb_test, 1])
        # generator_test_loss = combined.evaluate(
        #     [noise, sampled_labels.reshape((-1, 1))],
        #     [trick, sampled_labels], verbose=False)
        sampled_labels_disc1 = np.random.randint(0, class_num, nb_test)
        sampled_labels_disc1 = to_categorical(sampled_labels_disc1, label_dim)

        sampled_labels_disc2 = np.random.randint(0, class_num, nb_test)
        sampled_labels_disc2 = to_categorical(sampled_labels_disc2, label_dim)
        ...
        generator_test_loss = combined.evaluate(
            ...
            [trick, sampled_labels_disc], verbose=False)
