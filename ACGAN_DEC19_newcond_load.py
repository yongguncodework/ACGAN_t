...
load_epoch = 887  # Decide which epoch to reload weight, please check your file name
...
if __name__ == '__main__':
    ...
    generator = build_generator(latent_size+label_dim)

    latent = Input(shape=(latent_size,))
    image_class = Input(shape=(label_dim,), dtype='int32')
    gen_input = Input(shape=(latent_size+label_dim,))
    ...
    combined = Model([gen_input], [fake, aux])
    ...
    #
    #         # gen_input_sample = np.concatenate((noise, sampled_labels))
    #         gen_input_sample = np.concatenate([noise, sampled_labels], axis=1)
    #         a = sampled_labels.reshape((-1, 1))
    #         # generated_images = generator.predict(
    #         #     [noise, sampled_labels.reshape((-1, 1))], verbose=0)
    #         generated_images = generator.predict(
    #             [gen_input_sample], verbose=0)
    #
    ...
    #         epoch_gen_loss.append(combined.train_on_batch(
    #             [gen_input_sample], [trick, sampled_labels_disc]))
    #
    ...
    #     gen_input_sample = np.concatenate([noise, sampled_labels], axis=1)
    #     generated_images = generator.predict(
    #         [gen_input_sample], verbose=False)
    #
    ...
    #                    'YGfixed/Chair/ACGAN/model_save/params_discriminator_epoch_{0:03d}.hdf5'.format(load_epoch), True)
        ...
        y_test = y_train[0:100, 0:label_dim]
        mulnum = 0
        sampled_labels = y_test[16*mulnum:16*(mulnum+1), 0:label_dim]
        # sampled_labels = y_test[16:32, 0:label_dim]
        gen_input_sample = np.concatenate([noise, sampled_labels], axis=1)
        generated_images = generator.predict([gen_input_sample]).transpose(0, 2, 3, 1)
