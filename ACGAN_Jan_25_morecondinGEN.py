...
from keras.layers import Input, Dense, Reshape, Flatten, Embedding, Dropout, BatchNormalization, concatenate
...
def build_generator(latent_size):
    ...
    # y = np.expand_dims(image_class, axis=1)
    ...
    # cnn = concatenate([cnn, image_class], axis=1)
    # cnn.add(concatenate([cnn, feav]))


    cnn.add(Conv2DTranspose(192, kernel_size=5, strides=2, padding='same', activation='relu',
                            kernel_initializer='glorot_normal', bias_initializer='Zeros'))
    cnn.add(BatchNormalization())
    cnn.layers.append(image_class)
...
def build_discriminator():
    ...
    label_embedding = Flatten()(Embedding(4, np.prod([3, 64, 64]))(label))/4
