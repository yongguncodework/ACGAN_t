...
if __name__ == '__main__':
    ...
    adam_beta_1 = 0.5
    # adam_beta_1 = 0.45
    data_file_name1 = 'GAN_RSVP_NORM32.mat'
    # DATA_FOLDER_PATH = '/media/zijing/YG_Storage/ARL_BCIT/Processed EEG Data/X2 RSVP Expertise/YGfixed/Chair/FV/'

    DATA_FOLDER_PATH1 = '/media/eeglab/YG_Storage/ARL_BCIT/Processed_EEG_Data/X2 RSVP Expertise/' \
                        'YGfixed/Chair/Correct_Data/forGAN/this/'

    FILE_PATH1 = DATA_FOLDER_PATH1 + '/' + data_file_name1

    # mat_2 = scipy.io.loadmat(FILE_PATH)
    mat_1 = h5py.File(FILE_PATH1)
    mat_1.keys()

    EEG_data_res = mat_1['train_x']
    EEG_data_res = np.reshape(EEG_data_res, [880, 1024])
    ...
    for epoch in range(nb_epochs):
        ...
        for index in range(nb_batches):
            ...
            EEG_train = EEG_data_res[index * batch_size:(index + 1) * batch_size]
            ...
            for train_ix in range(3):
                if index % 30 != 0:
                    X_real = image_batch
                    # Label Soomthing
                    y_real = np.random.uniform(3.0, 1.2, size=(batch_size, 1))
                    ...
                    y_fake = np.random.uniform(0.0, 1.0, size=(batch_size, 1))
                    ...
                    X_real = image_batch
                    y_real = np.random.uniform(0.0, 1.0, size=(batch_size, 1))
                    # aux_y1 = label_batch.reshape(-1, )
                    aux_y1 = label_batch_one
                    ...
                    # Label Soomthing
                    X_fake = generated_images
                    y_fake = np.random.uniform(3.0, 1.2, size=(batch_size, 1))
                    # aux_y2 = sampled_labels_disc
                    aux_y2 = sampled_labels_disc
            ...
            trick = np.random.uniform(3.0, 1.2, size=(batch_size,))

            # epoch_gen_loss.append(combined.train_on_batch(
            #     [noise, sampled_labels.reshape((-1, 1))], [trick, sampled_labels]))
            epoch_gen_loss.append(combined.train_on_batch(
                EEG_train, [trick, sampled_labels_disc1]))
        ...
        generator_test_loss = combined.evaluate(
            EEG_combine,
            [trick, sampled_labels_disc1], verbose=False)
        ...
        EEG_sample = EEG_data_res[0:16, :]
        # sampled_labels = np.array([
        #     [i] * 4 for i in range(4)
        # ]).reshape(-1, 1)
        # y_test = y_train[0:100, 0:label_dim]
        # sampled_labels = y_test[0:16, 0:label_dim]
        generated_images = generator.predict(EEG_sample).transpose(0, 2, 3, 1)
