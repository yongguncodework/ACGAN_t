...
def build_discriminator():
    ...
    aux = Dense(4, activation='softmax', name='auxiliary',
                kernel_initializer='glorot_normal', bias_initializer='Zeros')(features)
...
if __name__ == '__main__':
    ...
    for epoch in range(nb_epochs):
        ...
        for index in range(nb_batches):
            ...
            # sampled_labels = np.random.randint(0, 2, size=batch_size)
            # sampled_labels = to_categorical(sampled_labels, label_dim)
            sampled_labels = np.random.normal(-1, 1, size=(batch_size, 4))
            ...
            # sampled_labels = to_categorical(sampled_labels, label_dim)
            sampled_labels = np.random.normal(-1, 1, size=(2* batch_size, 4))

            # we want to train the generator to trick the discriminator
            # For the generator, we want all the {fake, not-fake} labels to say
            # not-fake
            trick = np.random.uniform(0.7, 1.2, size=(2 * batch_size, 1))
        ...
        # sampled_labels = to_categorical(sampled_labels, label_dim)
        sampled_labels = np.random.normal(-1, 1, size=(nb_test, 4))
        generated_images = generator.predict(
        ...
        # sampled_labels = to_categorical(sampled_labels, label_dim)
        sampled_labels = np.random.normal(-1, 1, size=(2 * nb_test, 4))
