...
if __name__ == '__main__':
    ...
    # EEG_data = np.transpose(EEG_data, (0, 2, 3, 1))
    # EEG_data = EEG_data[:,:,:,1]

    # EEG_data_res = np.reshape(EEG_data, [880, 1024])
    # for idx in range(0, 880):
    #     EEG_data_res[idx] = EEG_data
    ...
    for epoch in range(nb_epochs):
        ...
        for index in range(nb_batches):
            ...
            EEG_data_input = EEG_data_res[index * batch_size:(index + 1) * batch_size]
            ...
            epoch_gen_loss.append(combined.train_on_batch(
                [noise], [trick, sampled_labels_disc]))
        ...
        # ]).reshape(-1, 1)
        EEG_data_res_sample = EEG_data_res[0:16]
        y_test = y_train[0:100, 0:label_dim]
        # sampled_labels = y_test[0:16, 0:label_dim]
        generated_images = generator.predict([EEG_data_res_sample]).transpose(0, 2, 3, 1)
