...
def build_generator(latent_size):
    ...
    cnn = Sequential()
    cnn.add(Dense(384 * 8 * 8, input_dim=latent_size+4, activation='relu',
    ...
    cls = Flatten()(Embedding(4, 25,
                              embeddings_initializer='glorot_normal')(image_class))

    # hadamard product between z-space and a class conditional embedding
    h = layers.multiply([latent, cls])
    # xc = np.concatenate((latent, latent), axis=-1)

    # noise_emb =Dense(100, activation='relu')(latent)
    # label_emb =Dense(1000, activation='relu')(image_class)
    # xc = Concatenate([latent, image_class], axis=-1)
